...
void WorldRenderer::allocateResources(glm::uvec2 swapchain_resolution, vk::Format swapchain_format)
{
  ...
  colorBuffer = ctx.createImage(etna::Image::CreateInfo{
    .extent = vk::Extent3D{resolution.x, resolution.y, 1},
    .name = "color_buffer",
  ...
  colorSampler = etna::Sampler(etna::Sampler::CreateInfo{
...
void WorldRenderer::loadShaders()
{
  ...
  etna::create_program(
    ...
    {AA_RENDERER_SHADERS_ROOT "static_mesh.frag.spv",
     AA_RENDERER_SHADERS_ROOT "static_mesh.vert.spv"});
  etna::create_program("static_mesh", {AA_RENDERER_SHADERS_ROOT "static_mesh.vert.spv"});

  etna::create_program(
    "terrain",
    {AA_RENDERER_SHADERS_ROOT "terrain.vert.spv",
     AA_RENDERER_SHADERS_ROOT "terrain.tesc.spv",
     AA_RENDERER_SHADERS_ROOT "terrain.tese.spv",
     AA_RENDERER_SHADERS_ROOT "terrain.frag.spv"});


  etna::create_program(
    "grass_placement", {AA_RENDERER_SHADERS_ROOT "grass_placement.comp.spv"});

  etna::create_program(
    "grass_render",
    {AA_RENDERER_SHADERS_ROOT "grass.vert.spv",
     AA_RENDERER_SHADERS_ROOT "grass.frag.spv"});

  etna::create_program(
    "clipmap_update",
    {AA_RENDERER_SHADERS_ROOT "clipmap_update.vert.spv",
    AA_RENDERER_SHADERS_ROOT "clipmap_update.frag.spv"});

  etna::create_program(
    "fxaa", 
    {AA_RENDERER_SHADERS_ROOT "fxaa.vert.spv",
...
void WorldRenderer::renderFxaa(vk::CommandBuffer cmd_buf, vk::Image target_image, vk::ImageView target_image_view)
{
  etna::set_state(cmd_buf, colorBuffer.get(),
    vk::PipelineStageFlagBits2::eFragmentShader,
    vk::AccessFlagBits2::eShaderSampledRead,
    vk::ImageLayout::eShaderReadOnlyOptimal,
    vk::ImageAspectFlagBits::eColor);
  ...
  auto programInfo = etna::get_shader_program("fxaa");
  auto set = etna::create_descriptor_set(
    programInfo.getDescriptorLayoutId(0), cmd_buf,
    {etna::Binding{0, colorBuffer.genBinding(
       colorSampler.get(),
       vk::ImageLayout::eShaderReadOnlyOptimal)}});
  ...
  pc.rcpFrame = glm::vec2(1.0f / resolution.x, 1.0f / resolution.y);
  pc.mode = aaMode;
...
void WorldRenderer::renderWorld(vk::CommandBuffer cmd_buf, vk::Image target_image, vk::ImageView target_image_view)
{
...

  {
    ...
    etna::RenderTargetState renderTargets(
      cmd_buf,
      {{0, 0}, {resolution.x, resolution.y}},
      {{.image = colorBuffer.get(), .view = colorBuffer.getView({})}},
  ...
  renderFxaa(cmd_buf, target_image, target_image_view);
}
...
void WorldRenderer::drawGui()
{
  ...
  const char* aaModes[] = { "No AA", "FXAA Basic", "FXAA 3.11" };
  ImGui::Combo("AA Mode", &aaMode, aaModes, 3);
