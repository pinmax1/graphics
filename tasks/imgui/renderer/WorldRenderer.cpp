...
{
  auto retval = etna::get_context().getDevice().waitIdle();
  if (retval != vk::Result::eSuccess) {
    spdlog::warn("Error in wait idle!");
  }
...
void WorldRenderer::drawGui()
{
  ...
  if (ImGui::Button("Load Baked Scene"))
  {
    loadScene(scenePathBuffer);
  }

  if (ImGui::Button("Bake and Load Scene"))
  {
    bakeModel(scenePathBuffer);
    const std::filesystem::path modelPath = scenePathBuffer;
    const std::filesystem::path modelDir  = modelPath.parent_path();
    const std::filesystem::path modelName = modelPath.stem();
    const std::filesystem::path bakedModelPath = modelDir / (modelName.string() + "_baked.gltf");
    loadScene(bakedModelPath);
  }
